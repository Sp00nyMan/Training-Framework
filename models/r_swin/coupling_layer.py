...
class CouplingLayer(nn.Module):
    ...
    """
    def __init__(self, in_channels, mid_channels, num_blocks, mask_type, reverse_mask, image_size):
        ...
        self.st_net = SwinTransformer(img_size=image_size, patch_size=2, in_chans=in_channels, out_chans=out_channels, 
                                      embed_dim=256, window_size=4, depths=[4], num_heads=[8])
    ...
    def forward(self, x, sldj=None, reverse=True):
        if self.mask_type == MaskType.CHECKERBOARD:
            ...
            x_b = x * b
            st = self.st_net(x_b) # Has the shape of x_b
